/* ============================================================================
   01 — תיקון אופרטורים הפוכים ב-stg.MergeOrdersData
   ----------------------------------------------------------------------------
   הבעיה:  בבלוק WHEN MATCHED, שני תנאים מתוך 12 השתמשו ב-'=' במקום ב-'<>'.
           בשרשרת OR הם מתקיימים כשהקטגוריה *לא* השתנתה — כלומר כמעט תמיד —
           ולכן ה-UPDATE רץ על כל 9,820 השורות בכל ריצה שעתית.
   התיקון: שני תווים. שורות 171-172 בלבד. שאר הפרוצדורה זהה למקור ב-git.
...
/* --- שלב 2: גיבוי ההגדרה הנוכחית. שמרו את הפלט לקובץ. ------------------ */
SELECT m.definition
...
/* --- שלב 4: אימות שהתיקון נכנס ------------------------------------------- */
SELECT
    CASE WHEN m.definition LIKE '%COALESCE(dest.[MainCategoryId],0) <> %'
          AND m.definition LIKE '%COALESCE(dest.[SecondaryCategoryId],0) <> %'
         THEN 'OK — שני האופרטורים תוקנו'
         ELSE 'FAIL — התיקון לא הוחל' END AS fix_status
FROM sys.sql_modules AS m
JOIN sys.objects     AS o ON o.object_id = m.object_id
JOIN sys.schemas     AS s ON s.schema_id = o.schema_id
WHERE s.name = 'stg' AND o.name = 'MergeOrdersData';
...
/* --- שלב 5: מדידה אחרי -----------------------------------------------------
   הריצו את השאילתה מ'שלב 1' שוב, אחרי שהג'וב CalibratorMainLoadProd רץ פעם אחת.
   הצפי: updated_last_2h יירד מ~9,820 לעשרות בודדות.
   אם הוא נשאר גבוה — יש סיבה נוספת לעדכונים, ויש לחקור לפני שממשיכים.
   --------------------------------------------------------------------------- */
