...
/*  MBA-944: on the packing screen the status comes from the DELIVERY NOTE.

    OrderWorkPlans.OrderOverallStatusId is essentially never advanced - 1,239 orders sit on
    'WaitingForCalibration' and exactly one on anything else - so every packing row reported
    'waiting for calibration'. That is not one of the five statuses the packing badge knows
...
SELECT @AwaitConfHeb = s2.StatusDescriptionHEB,
       @AwaitConfEng = s2.StatusDescriptionENG
FROM [dbo].[Statuses] as s2
JOIN [dbo].[StatusesCategories] as sc2 ON s2.StatusCategoryId = sc2.StatusCategoryId
WHERE sc2.StatusDescriptionENG = 'OrderStatus' AND s2.StatusDescriptionENG = 'AwaitingConfirmation';
...
'SELECT
	...
	,'
/*  Scoped to the packing page, exactly like the delivery-note filter further down, so no other
...
,CASE WHEN @Page = N'packing' THEN CONCAT(
    ...
    ,ordst.[StatusDescriptionENG] as OrderStatusENG' END, '
