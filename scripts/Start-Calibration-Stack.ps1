...
# Resolve repo/install root: works whether this script sits at the root (installed layout /
# legacy dev) or under scripts\ (dev). Installed behavior is unchanged.
$root = $PSScriptRoot
if (-not (Test-Path (Join-Path $root 'Systems')) -and -not (Test-Path (Join-Path $root 'consolehost'))) {
