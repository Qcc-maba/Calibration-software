...
BEGIN
	...
	FROM    dbo.Units AS Units_1 RIGHT OUTER JOIN
			dbo.MeasurementDevicesSubClass INNER JOIN
			dbo.MeasurementDevices ON dbo.MeasurementDevicesSubClass.ID = dbo.MeasurementDevices.SubClass INNER JOIN
			dbo.MeasurementDevicesMainClasses ON dbo.MeasurementDevices.MainClass = dbo.MeasurementDevicesMainClasses.Id INNER JOIN
			dbo.Calibrators INNER JOIN
			dbo.Users ON dbo.Calibrators.UserId = dbo.Users.ID ON dbo.MeasurementDevices.CalibratorId = dbo.Calibrators.ID LEFT OUTER JOIN
			...
			dbo.Units ON dbo.MeasurementDevices.Unit = dbo.Units.ID ON Units_1.ID = dbo.MeasurementDevices.WorkRangeUnit
