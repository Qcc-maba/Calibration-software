...
| Service fails to start, **error 1053** ("did not respond in a timely fashion", 30s) | The binary is a plain console web app, not a service host: it starts, binds its port, and never signals the SCM. Nothing is wrong with the config, the account or the database, and the logs show a clean startup. It needs `builder.Host.UseWindowsService(...)` plus the `Microsoft.Extensions.Hosting.WindowsServices` package. `Systems/InstructionAssistant` is the working reference; `CustomerPortalApi` shipped without both and lost a day to it. |
